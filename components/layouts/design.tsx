...
const DesignLayout: FC<DesignLayoutProps> = ({ name, selected, children }) => {
  ...
  return (
    <BaseLayout name={name} pageSlug="/design">
      ...
      above/below the article content is symmetrical. We do this instead of
      `py-6` to correctly position `<DesignNavBar />`, as well as the article
      content on small screens. This positioning issue is not present on larger
      screens so we have a breakpoint to reset it. For similar reasons, the
      `x` padding is set to 0 on small devices is set to 0. */}
