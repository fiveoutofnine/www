import Link from 'next/link';
import { useEffect, useState } from 'react';

import { ToastButton } from './component/toast.mdx';
import clsx from 'clsx';
import { ArrowRight, ChevronRight, Github } from 'lucide-react';

import DesignLayout from '@/components/layouts/design';
import { Badge, Button, IconButton, Select, Tooltip } from '@/components/ui';

# Design

An open-source design system aiming to be accessible, functional, and provide a consistent feel across this site.

<div className="grid grid-cols-2 flex-wrap gap-2 min-[432px]:flex">
  <Button
    className="col-span-2 w-full min-[432px]:w-fit"
    href="https://github.com/fiveoutofnine/www"
    leftIcon={<Github />}
    children="fiveoutofnine/www"
    newTab
  />
  <Button
    className="col-span-1 w-full min-[432px]:w-fit"
    variant="outline"
    href="/design/color"
    rightIcon={<ChevronRight />}
    children="Color"
  />
  <Button
    className="col-span-1 w-full min-[432px]:w-fit"
    variant="outline"
    href="/design/libraries"
    rightIcon={<ChevronRight />}
    children="Libraries"
  />
</div>

## Components

<div className="grid grid-cols-2 gap-4 md:grid-cols-4">
  <ComponentFeature href="/design/component/badge">
    <Badge children="Badge" />
  </ComponentFeature>
  <ComponentFeature href="/design/component/button">
    <Button children="Button" />
  </ComponentFeature>
  <ComponentFeature href="/design/component/select">
    <Select children={<option children="Select" />} />
  </ComponentFeature>
  <ComponentFeature href="/design/component/toast">
    <ToastButton children="Toast" />
  </ComponentFeature>
  <ComponentFeature href="/design/component/tooltip">
    <Tooltip content="5/9 moment">
      <Button children="Tooltip" />
    </Tooltip>
  </ComponentFeature>
</div>

export const ComponentFeature = ({ href, children }) => {
  const [isMounted, setIsMounted] = useState(false);
  useEffect(() => setIsMounted(true), []);
  const isMobile = isMounted ? /iPhone|iPad|iPod|Android/i.test(navigator.userAgent) : false;
  const containerStyles =
    'flex h-32 w-full items-center justify-center rounded-xl border border-gray-7 bg-gray-2';
  if (isMobile) {
    return (
      <Link
        className={clsx(
          containerStyles,
          'no-underline focus-visible:outline-none focus-visible:ring focus-visible:ring-blue-9',
        )}
        href={href}
        tabIndex={0}
      >
        {children}
      </Link>
    );
  }
  return (
    <div
      className={clsx(containerStyles, 'group relative transition-colors hover:border-gray-8')}
      tabIndex={-1}
    >
      {children}
      <IconButton
        variant="ghost"
        className="absolute bottom-2 right-2 hidden animate-in fade-in group-hover:flex"
        href={href}
      >
        <ArrowRight />
      </IconButton>
    </div>
  );
};

export default (props) => <DesignLayout name="Introduction" selected="/design" {...props} />;
