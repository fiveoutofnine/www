...
Within team "on-chain," there were some initial discussions about what art to create: should it be something playful, a performance art, a custom smart contract, etc.?
...
Eventually, we decided came up with the idea of "adopting a hyphen," where the open edition NFT (aptly named "adoption tickets") could be burned in exchange for a generative ASCII character NFT.
...
<div className="flex w-full items-center justify-center rounded-xl border border-gray-6 bg-gray-2 p-8">
  <div>hi</div>
  ```tsx showLineNumbers={false} containerized={false}
  {
    "name": "candle-pen",
    "role": "npc",
    "team": "green",
