...
I realized that because the font was mono-spaced, and the characters appeared in a grid, I could construct 1 string containing all the characters inside a `<pre>` element to preserve the whitespace.
This way, a lot of the redundancy from the `<text>` elements' opening and closing tags elements would be removed:
...
Now, with this setup, the problem became: look through indices `[0, 23 * 11 - 1]`, and at each index, draw a character for either the background or for the hyphen guy while maintaining whitespace for both strings.

Luckily, since $23\cdot11-1\leq256$, an easy and efficient way to accomplish this was to use a `uint256` bitmap[™](https://fiveoutofnine.com/blog/basic-bit-manipulation), where `0` at the corresponding index indicates an instruction to draw a hyphen guy character, and `1` indicates an instruction to draw a background character.
...
One obstacle with this structure was that depending on whether the hyphen guy had the "hat" and "chest" traits set, the corresponding indices could be empty, so the bitmap would have to be adjusted accordingly.

<div className="w-full flex justify-center">
  <DynamicBitmapGraphic />
</div>

To get around this, I reserved 0 as a special flag for the hat/chest traits, and `XOR`'d the bits where the hat/chest characters are located.
This way, if the hat/chest traits were set, the corresponding bit would flip to `0` (i.e. draw a hyphen guy character), and if not, the bit would remain `1`:
...
After this, I needed to implement the "intensity" trait, which selects a random percentage value `intensity` (roughly between 20% and 80%) to fill the background.
I reused the bitmap technique here.

1. Fill a `uint256[]` with `intensity` `1`s.
2. Shuffle it.
3. Generate bitmap.
