...
We can already kinda see a single 2D Perlin map resembles some natural geographic formation, like a topographic map.
By taking the values from a Perlin map as mapping for a single _property_, we can easily generate multiple maps and combine values from all of them to get smooth transitions between biomes.
...
For example, in a game with complex terrain generation like Minecraft, we might want to take into account many properties such as continentalness, erosion, peaks and valleys, temperature, and humidity.
We can also see from the image above that each property has a unique random map generator.

In my case, I just needed to select between 1 biome value out of 11 total possible, so I went with just 2 2D Perlin maps: 1 for rainfall and 1 for temperature.
