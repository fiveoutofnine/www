...
We can already kinda see how a single 2D Perlin map resembles some natural, geographic formation, like a topographic map displaying some geographic property like altitude.
By mapping the values from a single map to a single _property_, we can build a simple framework to introduce more complex generation where we use multiple maps and combine values from all of them to get smooth transitions between biomes.
...
For example, in a game with complex terrain generation like Minecraft, we might want to take into account many properties such as continentalness, erosion, peaks and valleys, temperature, and humidity at each position.
We can also see from the image above that each property has a unique random map generator.

In my case, I just needed to select 1 biome value out of 11 total at each tile, so I went with just 2 2D Perlin maps: 1 for rainfall and 1 for temperature.
Then, I came up with the following mapping:
