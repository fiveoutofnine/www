...
import { ANDGraphic, ORGraphic, SHLGraphic, SHRGraphic, XORGraphic } from './graphics';
...
>
  <Grid.Cell>
    ...
    <div className="w-full flex items-center justify-center p-4 my-3 border border-gray-6 rounded-lg">
      <SHRGraphic />
    </div>
...
Once we achieve both, we can look towards using as few bits as possible because storage costs are (generally) the most expensive on the EVM.
...
However, `uint8[9] board` uses a lot of unnecessary reads/writes, and we don't need 8 bits to store data for a position on the board.
Since there are only 3 valid states, 2 bits ($2^2=4$) are enough.
...
```
*See: [How to turn `if` blocks into 3 bit operations for small gas savings.](https://x.com/fiveoutofnine/status/1491894008049811458)*
...
Source: [*Tweet by 0xSubway*](https://x.com/0xSubway/status/1506130207417126912)
...
* [emre](https://x.com/emrecolako)
* [virtual](https://x.com/9dd4134)
* [0xsomnus](https://x.com/0xsomnus)

If you found this helpful, consider following me on Twitter [@fiveoutofnine](https://x.com/fiveoutofnine).
